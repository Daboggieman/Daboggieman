...
func renderTableView(s *Stats) string {
	...
	if len(blocks) > 0 {
		...
		for _, r := range blocks {
			...
			fmt.Fprintf(&b, "| [%s](https://github.com/%s/%s) | %s | %s | %s | %s | %s |\n",
				r.Name, s.Login, r.Name, commas(r.Commits), humanBytes(r.SizeKB*1024),
				commas(r.Stars), r.PushedAt.Format("2 Jan 2006"), state)
		}
		fmt.Fprintf(&b, "\n<sub>Building height is commits on the default branch, footprint is repo size, "+
			"and a lit facade means pushed within %s.</sub>\n", cityAge())
	}
